<template>
    <div class="area">
        ...
        <div v-for="set in datasets" :key="set.id" @click="selectedDataset = set.id">
            <div class="row v-center">
                <div class="btn" :class="{ active: selectedDataset === set.id }">{{ `${set.name} (#${set.count})` }}</div>
        ...
        <div class="btn" @click="handleChangeDataset(selectedDataset, imgCount)">load Dataset</div>
        </div>
    </div>
</template>
...
export default {
    ...
    methods: {
        changeImgCount({ target }) {
            ...
            this.imgCount = +target.value < 500 ? 500 : +target.value;
        },
    },
};
</script>
